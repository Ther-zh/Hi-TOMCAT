...
# ====================== 新增1：加载预生成的路径映射（只加载1次） ======================
PATH_MAP_PATH = "/root/tomcat/TOMCAT/ut_zappos_path_map.pkl"
with open(PATH_MAP_PATH, "rb") as f:
    IMAGE_PATH_MAP = pickle.load(f)
...
class ImageLoader:
    ...
    def __call__(self, img):
        # 提取图片文件名（如从"Faux.Leather_Boots.Ankle/7965307.5291.jpg"提取"7965307.5291.jpg"）
        img_filename = os.path.basename(img)
        
        # 优先查映射（适配实际路径）
        if img_filename in IMAGE_PATH_MAP:
            file = IMAGE_PATH_MAP[img_filename]
        else:
            # 兜底：用原有拼接路径（兼容少量匹配不到的情况）
...
class CompositionDataset(Dataset):
    ...
    def get_primitives_mapping(self):
        """返回属性名、对象名、组合→(attr_idx, obj_idx)映射"""
        attr_names = self.attrs  # 所有属性名称列表
        obj_names = self.objs    # 所有对象名称列表
        # 组合对映射：每个组合对应（属性索引，对象索引）
        comp_to_prim = [(self.attr2idx[attr], self.obj2idx[obj]) for attr, obj in self.pairs]
        return attr_names, obj_names, comp_to_prim
