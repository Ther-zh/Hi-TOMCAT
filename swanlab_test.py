import argparse
import copy
import random  # 补充缺失的导入（原代码用了random.seed但没导入）
...
from swan_test_tomcat import predict_logits_text_first_with_tomcat
...
def predict_logits_text_first(model, dataset, config):
    """Function to predict the cosine similarities (text first mode)"""
    model.eval()
...
def test(
        ...
        all_pair_gt,
        config):
    """Function computes accuracy on the validation/test dataset."""
    predictions = {
...
if __name__ == "__main__":
    ...
    # 2. 替换wandb初始化为swanlab初始化
    if config.use_wandb:  
        # 若仍用原参数名use_wandb，可保留为if config.use_wandb:
        # SwanLab初始化（project名称保持和原wandb一致）
        swanlab.init(
            project='TTA-' + config.dataset,
            config=vars(config),  # 自动记录配置参数（SwanLab特色）
            mode=config.swanlab_mode if hasattr(config, 'swanlab_mode') else 'online'  # 对应原wandb_net
        )
    ...
    # 3. 替换wandb.log为swanlab.log
    if config.open_world is False and config.use_wandb:  # 同理：若用原参数名则保留config.use_wandb
        swanlab.log(test_stats)  # SwanLab自动记录所有评估指标
